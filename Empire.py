...
def run_empire(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
            ...
            for w in instance.Scenario:
                for (s,h) in instance.HoursOfSeason:
                    ...
                    my_string.extend([value(sum(-instance.storCharge[n,b,h,i,w] for b in instance.Storage if (n,b) in instance.StoragesOfNode)), 
                        ...
                        value(instance.dual[instance.FlowBalance[n,h,i,w]]/(instance.seasScale[s]*instance.sceProbab[w])),
    ...
    for i in instance.PeriodActive:
        for w in instance.Scenario:
            ...
            if EMISSION_CAP:
                my_string.extend([value(instance.dual[instance.emission_cap[i,w]]/(instance.sceProbab[w]*1e6)),value(instance.CO2cap[i]*1e6)])
            else:
            ...
            value(sum(instance.dual[instance.FlowBalance[n,h,i,w]]/(instance.seasScale[s]*instance.sceProbab[w]) for n in instance.Node for (s,h) in instance.HoursOfSeason)/value(len(instance.HoursOfSeason)*len(instance.Node))),
            ...
            for (s,h) in instance.HoursOfSeason:
                for n in instance.Node:
                    f = row_write(f, dict_countries_reversed[str(n)], "Price|Secondary Energy|Electricity", "US$2010/GJ", seasonhours[h-1], \
                        [value(instance.dual[instance.FlowBalance[n,h,i,w]]/(GJperMWh*instance.seasScale[s]*instance.sceProbab[w])) for i in instance.PeriodActive], Scenario+"|"+str(w)+str(s))
        for g in instance.Generator:
